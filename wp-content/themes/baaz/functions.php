...
add_action( 'after_setup_theme', 'baaz_i18n' );
/**
 ...
 */
function baaz_i18n() {
    load_child_theme_textdomain( 'baaz', get_stylesheet_directory() . '/languages' );
}
...
function baaz_enqueue_assets() {
	wp_enqueue_style( 'baaz', get_stylesheet_uri() );
	wp_style_add_data( 'baaz', 'rtl', 'replace' );
}
...
add_filter( 'wp_headers', 'baaz_keep_ie_modern' );
function baaz_keep_ie_modern( $headers ) {
...
add_action( 'genesis_entry_header', 'baaz_do_post_image', 5 );
function baaz_do_post_image() {
	global $post;
	if( ! is_singular( 'post' ) ) {
		return;
	}
	$feature_image = get_post_meta($post->ID, '_baaz_post_image_url', true);
	if ( $feature_image ) {
...
function custom_search_button_text( $text ) {
    return esc_attr( 'Go', 'baaz' );
}


// Modify the author box display
add_filter( 'genesis_author_box', 'baaz_author_box' );
function baaz_author_box() {
	$authinfo = '';
	...
	if( !empty( $authdesc ) ) {
		...
		$authinfo .= '<h3 class="author-box-title">' . __( 'About ', 'baaz' ) . get_the_author_meta( 'display_name' ) . '</h3>';
...
function post_meta_filter( $post_meta ) {
    ...
    $post_meta = '[post_categories sep=", " before="<b>' . __( 'Categories:', 'baaz' ) . '</b> "] [post_tags sep=", " before="<b>' . __( 'Tags:', 'baaz' ) . '</b> "]';
    return $post_meta;
}

// Add Read More button to blog page and archives
add_filter( 'excerpt_more', 'baaz_add_excerpt_more' );
add_filter( 'get_the_content_more_link', 'baaz_add_excerpt_more' );
add_filter( 'the_content_more_link', 'baaz_add_excerpt_more' );
function baaz_add_excerpt_more( $more ) {
    return '<span class="more-link"><a href="' . get_permalink() . '" rel="nofollow">' . __( 'Read More', 'baaz' ) . '</a></span>';
}
 ...
 */
add_action( 'get_header', 'baaz_load_scripts' );
function baaz_load_scripts() {
...
add_theme_support ( 'genesis-menus' ,
	array (
		'primary'   => __( 'Primary Navigation Menu', 'baaz' ),
		'secondary' => __( 'Secondary Navigation Menu', 'baaz' ),
	)
);
...
function be_search_icons( $menu, $args ) {
	...
	$search = genesis_get_option( 'wsm_search', 'baaz-settings' );
	...
	get_search_form();
	$search = ob_get_clean();
	$menu_right  .= '<li class="search menu-item last"><span id="show_hide">' . __( 'Search', 'baaz' ) . '</span><div class="slidingDiv" style="display:none;">' . $search . '</div></li>';
...
function be_phone_info( $menu, $args ) {
	...
	$args = (array)$args;
	$phone = genesis_get_option( 'wsm_phone', 'baaz-settings' );
...
add_action( 'genesis_before_comments' , 'baaz_move_comments' );
function baaz_move_comments () {
...
genesis_register_sidebar( array(
	'id'			=> 'rotator',
	'name'			=> __( 'Rotator', 'baaz' ),
	'description'	=> __( 'This is the image rotator section.', 'baaz' ),
) );
genesis_register_sidebar( array(
	'id'			=> 'home-main',
	'name'			=> __( 'Home Main Section', 'baaz' ),
	'description'	=> __( 'This is the home page widget section.', 'baaz' ),
) );
genesis_register_sidebar( array(
	'id'			=> 'blog-sidebar',
	'name'			=> __( 'Blog Sidebar', 'baaz' ),
	'description'	=> __( 'This is the Blog Page Sidebar.', 'baaz' ),
) );
genesis_register_sidebar( array(
	'id'			=> 'page-sidebar',
	'name'			=> __( 'Page Sidebar', 'baaz' ),
	'description'	=> __( 'This is the Page Sidebar.', 'baaz' ),
) );
genesis_register_sidebar( array(
	'id'			=> 'footer-social',
	'name'			=> __( 'Footer Social Media', 'baaz' ),
	'description'	=> __( 'This is the Footer Social Media Icons Section.', 'baaz' ),
) );


// Remove Genesis Blog Template
add_filter( 'theme_page_templates', 'baaz_remove_genesis_page_templates' );
function baaz_remove_genesis_page_templates( $page_templates ) {
	unset( $page_templates['page_blog.php'] );
	return $page_templates;
}

// Add Posts Social Share
add_action( 'genesis_entry_footer', 'baaz_post_ssbp' );
function baaz_post_ssbp() {
