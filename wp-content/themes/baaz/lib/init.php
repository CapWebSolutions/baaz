...
function baaz_init() {
	...
	define( 'CHILD_THEME_NAME', 'BAAZ' );
	define( 'CHILD_THEME_URL', 'http://capwebsolutions.com/' );
	define( 'CHILD_THEME_VERSION', wp_get_theme()->get( 'Version' ) );
	define( 'BAAZ_SETTINGS_FIELD', 'baaz-settings' );

	// Developer Information (do not remove)
	define( 'CHILD_DEVELOPER', 'Cap Web Solutions' );
	define( 'CHILD_DEVELOPER_URL', 'http://capwebsolutions.com/'  );
	...
	define( 'BAAZ_URL', get_stylesheet_directory_uri() );
	define( 'BAAZ_DIR', get_stylesheet_directory() );
	define( 'BAAZ_LIB', CHILD_URL . '/lib' );
	define( 'BAAZ_IMAGES', CHILD_URL . '/images' );
	define( 'BAAZ_ADMIN', CHILD_LIB . '/admin' );
	define( 'BAAZ_ADMIN_IMAGES', CHILD_LIB . '/images' );
	define( 'BAAZ_JS' , CHILD_URL .'/lib/js' );
	...
	add_action( 'wp_enqueue_scripts', 'baaz_load_fonts' );

	// Mobile menu
	include_once( CHILD_DIR . '/lib/functions/mobilemenu.php');

	// Enable Customizer Support for site title, description, and widgets
	add_action( 'customize_register', 'cws_customize_register' );
	add_theme_support( 'customize-selective-refresh-widgets' );

}

function cws_customize_register( WP_Customize_Manager $wp_customize ) {
	global $wp_version;
	if ( $wp_version < 4.5 ) {
		return;
	}
...
function baaz_google_fonts_url() {
    $fonts_url = '';
    ...
    */
    $yantramanav = _x( 'on', 'Yantramanav font: on or off', 'baaz' );

    /* Translators: If there are characters in your language that are not
    * supported by Prata, translate this to 'off'. Do not translate
    * into your own language.
    */
    $libre_baskerville = _x( 'on', 'Libre Baskerville font: on or off', 'baaz' );
...
function baaz_load_fonts() {
	wp_register_style( 'genericons', CHILD_URL . '/lib/genericons/genericons.css', array(), CHILD_THEME_VERSION, 'all' );
	wp_enqueue_style( 'genericons' );
	wp_enqueue_style( 'baaz-fonts', baaz_google_fonts_url(), array(), null );
	wp_enqueue_style( 'material-icons', 'https://fonts.googleapis.com/icon?family=Material+Icons', array(), null );
}

add_filter( 'http_request_args', 'baaz_dont_update_theme', 5, 2 );
/**
 * Don't Update Theme
 ...
 */
function baaz_dont_update_theme( $r, $url ) {
