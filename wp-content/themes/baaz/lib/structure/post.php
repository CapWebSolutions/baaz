...
add_filter( 'genesis_attr_body', 'cws_attributes_body' );
/**
 * If viewing blog post or archive, make this a Blog page type.
 */
function cws_attributes_body( $attributes ) {
...
add_filter( 'genesis_attr_entry', 'cws_attributes_entry' );
/**
 * Add Schema.org microdata to the entry.
 *
 * No need to change any of this.
 */
function cws_attributes_entry( $attributes ) {
...
add_action( 'genesis_entry_content', 'cws_extra_entry_meta', 5 );
/**
 * Extra recommended meta data about the entry
 */
function cws_extra_entry_meta() {
...
//add_action( 'genesis_entry_content', 'cws_entry_image', 8 );
/**
 * Output the entry image object.
 */
function cws_entry_image() {
...
function cws_entry_publisher() {
	...
	$publisher_name = 'Cap Web Solutions';
